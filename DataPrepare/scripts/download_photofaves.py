...
def producer(queue, group):
    ...
    enqueue photos whose faves has not been downloaded or failed to
    ...
    donelog = osp.join(pathname, gid+".faves.log.done")
    faillog = osp.join(pathname, gid+".faves.log.fail")
    try:
    ...
    while True:
        ...
        try:
            ...
            try:
                gid = item['gid']
                iid = item['id']
                print(gid, ": download photo favorites", iid)
                status, faves= flickrGetPhotoFaves(iid)
                with GLOCKS[gid]:
                    if status == False: # failed
                        with open(osp.join(PROJROOT, "assets", gid, gid+".faves.log.fail"), 'a') as f:
                            f.write(iid+"\n")
                        continue
                    with open(osp.join(PROJROOT, "assets", gid, gid+".faves.log.done"), 'a') as f:
                        f.write(iid+"\n")
                    with open(osp.join(PROJROOT, "assets", gid, gid+".faves"), 'a') as f:
                        json.dump({"id":iid, "person":faves}, f) # serialize in json format
                        f.write("\n")
...
## each group, the script tries to download photo favorites for each photo in its photo pool
## (i.e. photos saved in the photo list file `GID.pkl`). Retrieved information is saved in
## file `GID.faves`, one line per photo. A group whose photo infos exists will be skipped. 
...
## Files:
## GID.pkl, photo lists
## GID.faves, photo favorites, one item per line in format of json
## GID.faves.log.done, photo ids whose favorites has been retrieved
## GID.faves.log.fail, photo ids whose favorites has failed to be retrieved
if __name__ == "__main__":
